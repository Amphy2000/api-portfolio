...
async function getFuelPrices(state = null, fuelType = null) {
  ...
  let url = 'https://fuel-price-tracker.p.rapidapi.com/fuel-prices';
  const params = [];
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'x-rapidapi-host': 'fuel-price-tracker.p.rapidapi.com'
    }
  });
...
def get_fuel_prices(state=None, fuel_type=None):
    url = "https://fuel-price-tracker.p.rapidapi.com/fuel-prices"
    querystring = {}
    if state:
...
curl --request GET \
	--url 'https://fuel-price-tracker.p.rapidapi.com/fuel-prices?state=TX&type=diesel' \
	--header 'x-rapidapi-host: fuel-price-tracker.p.rapidapi.com' \
